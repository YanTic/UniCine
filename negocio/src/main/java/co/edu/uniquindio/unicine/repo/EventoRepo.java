...
public interface EventoRepo extends JpaRepository<Evento, Integer> {
    ...
    @Query("select e from Evento e where e.nombre = :nombre and e.fecha = :fecha and e.hora_inicio = :hora_inicio and e.hora_fin = :hora_fin and e.teatro.id = :idTeatro")
    Optional<Evento> verificarExistencia(String nombre, LocalDate fecha, LocalTime hora_inicio, LocalTime hora_fin, Integer idTeatro);
